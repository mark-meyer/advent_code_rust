    ...
    for &[u, v, start_dist] in tree.iter() {
        for &[u_next, v_next, end_dist] in tree.range_query(
            [
                u - cheat_length,
                v - cheat_length,
                start_dist + min_savings as isize,
            ],
            ...
            if start_dist + cheat < end_dist                                    // is the cheat is actually an improvement?
            && end_dist.abs_diff(start_dist) - (cheat as usize) >= min_savings  // avoid counting small savings
            {
                total += 1;
            }
        }
